(function($){
	...
	$(document).ready( function(){
		...
		if($('#et-top-navigation ul.nav').length)
		{
			et_duplicate_menu( $('#et-top-navigation ul.nav'), $('#et-top-navigation .mobile_nav'), 'mobile_menu0', 'et_mobile_menu' );
			$('#et-top-navigation #mobile_menu0').append( $( '#et-secondary-nav' ).clone().html() );
		}
		else
		{
			et_duplicate_menu( $('#et-secondary-menu ul'), $('#et-top-navigation .mobile_nav'), 'mobile_menu0', 'et_mobile_menu' );
		}
		...
		$('#et-secondary-menu ul').each(function() {
			var this_menu = $( this ),
				bg_color = this_menu.data( 'bg_color' );
			if ( bg_color ) {
				this_menu.find( 'ul ul' ).css( { 'background-color' : bg_color } );
		...
		/*function et_duplicate_menu( menu, append_to, menu_id, menu_class ){
			append_to.each( function() {
				...
				$this_menu.on( 'click', 'li a', function(event){
		...
		function et_duplicate_menu( menu, append_to, menu_id, menu_class ){
			append_to.each( function() {
				var $this_menu = $(this),$cloned_nav;

				menu.clone().attr('id',menu_id).removeClass().attr('class',menu_class).appendTo( $this_menu );
				$cloned_nav = $this_menu.find('> ul');
				//console.log($cloned_nav);
				$cloned_nav.find('.menu_slide').remove();
				$cloned_nav.find('li:first').addClass('et_first_mobile_item');
