<?php if (!defined('ABSPATH')) exit; // Exit if accessed directly
    ...
    function bkx_get_base_on_seat_callback(){
        ...
        if (isset($_POST['loc'])) {
             ...
             */
            if ($term == 'any' && bkx_crud_option_multisite('enable_any_seat') == 1):
                $args = array(
                    'posts_per_page' => -1,
                    'post_type' => 'bkx_base',
                    'post_status' => 'publish',
            ...
            if (isset($term) && $term != '') {
                ...
                if (!empty($objSeat) && !is_wp_error($objSeat)) {
                    ...
                    if (!empty($values) && !is_wp_error($values)) {
                        ...
                        $seat_is_pre_payment = isset($values['seatIsPrePayment']) ? esc_attr($values['seatIsPrePayment'][0]) : "N";
                    }
                }
                ...
                $bkx_seat_is_booking_prepayment =  apply_filters( 'bkx_seat_is_booking_prepayment', $seat_is_pre_payment );
                if($bkx_seat_is_booking_prepayment == "Y"){
                    $seat_is_pre_payment = "Y";
                }
            ...
            if (!empty($get_base_array)) {
                ...
                foreach ($get_base_array as $key => $value) {
                    ...
                    $arr_base['base_list'][$counter]['currency_sym'] = $currency;
                    $arr_base['base_list'][$counter]['currency_name'] = $currency_option;
    ...
    function bkx_get_addition_on_base_callback(){
        ...
        if (!empty($get_extra_array) && !is_wp_error($get_extra_array)) {
            ...
            foreach ($get_extra_array as $key => $value) {
                $extra_name = ucwords( $value->post_title );
                $extra_id = $value->ID;
    ...
    function bkx_get_total_price_callback(){
        ...
        $additionid = !empty($_POST['additionid']) ? array_map('sanitize_text_field', wp_unslash($_POST['additionid'])) : "";
        ...
        $booking_require_prepayment = apply_filters( 'bkx_seat_is_booking_prepayment'       , $booking_require_prepayment );
        $payment_type               = apply_filters( 'bxk_booking_prepayment_payment_type'  , $payment_type );
        $deposit_type               = apply_filters( 'bxk_booking_prepayment_deposit_type'  , $deposit_type );
        $percentage                 = apply_filters( 'bxk_booking_prepayment_percentage'    , $percentage );
        $fixed_amount               = apply_filters( 'bxk_booking_prepayment_fixed_amount'  , $fixed_amount );
    ...
    function bkx_get_duration_booking_callback()
    {
        ...
        $baseid = sanitize_text_field($_POST['baseid']);
        if(isset($_POST['additionid']) && !empty($_POST['additionid'])){
        ...
        if (is_numeric($mobonlyobj) && $mobonlyobj != 0) {
            ...
            $seat_is_pre_payment = isset($seatmeta['seatIsPrePayment']) ? esc_attr($seatmeta['seatIsPrePayment'][0]) : "N";
            $seat_is_pre_payment =  apply_filters( 'bkx_seat_is_booking_prepayment', $seat_is_pre_payment );
        ...
        //mobile only data
        if (isset($mobonlyobj)) {
            $mob_only_res = isset($objBase['base_is_mobile_only'][0]) ? $objBase['base_is_mobile_only'][0] : "";
        }
        ...
        if (isset($baseid)) {
            ...
            if ($base_time_option == "H") {
                ...
                $base_time = $objBase['base_hours'][0] + $base_minutes; //add base minutes to base hours
            }
            ...
            if (isset($additionid) && sizeof($additionid) > 0) {
                ...
                foreach ($objListAddition as $extra) {
                    ...
                    if ($objextra['addition_time_option'][0] == "H") {
                        ...
                        $objAddition[$extra->ID]['addition_time'] = $objextra['addition_hours'][0] + $addition_minutes;
                        //add addition minutes to addition hours
                    } else if ($extra->addition_time_option == "D") {
                    ...
                    } else if ($extra->addition_time_option == "M") {
        ...
        $total_tax = 0;
        $base_extended = isset($_POST['extended']) ? $_POST['extended'] : 0;
        $total_price = bkx_cal_total_price($baseid, sanitize_text_field($base_extended), $additionid);
        ...
        $payment_gateways = "";
        if(!empty($seat_is_pre_payment) && $seat_is_pre_payment == "Y"){
            $BkxPaymentCore = new BkxPaymentCore();
            $payment_gateways = $BkxPaymentCore->bkx_get_available_gateways_html();
        }
        $currency = bkx_get_current_currency();
        ...
        $arr_output['bkx_get_available_gateways_html'] = $payment_gateways;
        $arr_output['seat_is_pre_payment'] = $seat_is_pre_payment;
    ...
    function bkx_check_ifit_canbe_booked_callback() {
        ...
        $update_order_slot  = isset($_POST['update_order_slot']) ? sanitize_text_field($_POST['update_order_slot']) : "";
        $selected_order_id  = isset($_POST['selected_order_id']) ? sanitize_text_field($_POST['selected_order_id']) : "";
    ...
    function bkx_displaytime_options_callback(){
        ...
        $_POST['bookigndate'] = isset($_POST['bookigndate']) ? $_POST['bookigndate'] : $booking_default_date;
        if (isset($_POST['bookigndate'])) {
            ...
            if (isset($_POST['seatid']) && $_POST['seatid'] == 'any' && $service_id != '' && bkx_crud_option_multisite('enable_any_seat') == 1 && bkx_crud_option_multisite('select_default_seat') != ''):
                ...
                $seat_id = isset($_POST['seatid']) ? sanitize_text_field($_POST['seatid']) : "";
                $search = array('bookigndate' => $bookigndate, 'service_id' => $service_id, 'status' => $order_statuses, 'seat_id' => $seat_id, 'display' => 1);
                $BkxBooking = new BkxBooking();

                $objBookigntime = $BkxBooking->GetBookedRecords($search);
                if (!empty($objBookigntime)) {
                    if(isset($objBookigntime[0]->full_day)){
        ...
        $i = 0;
        $booking_slot = array();
        $objBookigntime = !empty($objBookigntime) || isset($objBookigntime) ? $objBookigntime : "";
        ...
        $time_available_color = isset($time_available_color) ? $time_available_color : '#368200';

        $time_selected_color = bkx_crud_option_multisite('bkx_time_selected_color');
        $time_selected_color = isset($time_selected_color) ? $time_selected_color : '#e64754';

        $time_unavailable_color = bkx_crud_option_multisite('bkx_time_unavailable_color');
        $time_unavailable_color = isset($time_unavailable_color) ? $time_unavailable_color : 'gray';

        $bkx_time_new_selected = bkx_crud_option_multisite('bkx_time_new_selected');
        $bkx_time_new_selected = isset($bkx_time_new_selected) ? $bkx_time_new_selected : "";
    ...
    function bkx_cancel_booking_callback(){
        ...
        if (isset($booking_record_id) && $booking_record_id != '' && $mode == 'cancelled'):
            ...
            $BkxBookingObj->add_order_note(sprintf(__(' %1$s ', 'bookingx'), $comment), $is_cust);
            if ($update_order) :
