	...
	loader.show();
		...
		else
		{
			...
			//if( booked_days == 'yes' && base_days != '' ) { 
				...
				if(collect_disable_days === undefined || collect_disable_days =='' ){ console.log('sterp asas')}
				else
				{
 ...
 *@return void
 */
function bkx_change_timepicker_val( date, inst)
{
	var booked_days = $('#id_booked_days').val();
	...
	if($("#id_booking_duration_insec").attr("display-date-picker") == "1")
	 {
		$( "#id_datepicker_extended" ).datepicker();
		 ...
		 $( "#id_datepicker" ).datepicker({minDate: today,beforeShowDay: bkx_disableSpecificWeekDays, onSelect: function( date, inst){bkx_change_timepicker_val(date, inst);}});
	 }
	 else
	 {
  ...
  $(function() {		
	...
	$('#id_base_selector').bind('change', function(){
		...
		$.post(url_obj.bkx_ajax_url, { baseid: base_temp, seat_id : seat_id, action : 'bkx_get_addition_on_base' }, function(data) {
			...
			{
			  //console.log(temp_obj[x]['seat_id']+temp_obj[x]['seat_name']+temp_obj[x]['seat_price']);
			  temp_option += '<li class="gchoice_5_1"><input name="addition_name[]" class="addition_name_class" id="addition'+temp_obj[x]['addition_id']+'" type="checkbox" value="'+temp_obj[x]['addition_id']+'" id="choice_5_1" tabindex="4" onchange="bkx_get_total_price(this);"><label for="choice_5_1">'+temp_obj[x]['addition_name']+' - $'+temp_obj[x]['addition_price']+' - '+temp_obj[x]['addition_time']+'</label></li>';
			}
...
function bkx_get_total_price(thisval)
{
	...
	$.post(url_obj.bkx_ajax_url, { action: 'bkx_get_total_price', seatid : seat_temp, baseid: base_temp,additionid : addition_list,extended : base_extended}, 	function(data) {
		...
		$('span#total_time_of_services_formatted').html(data['total_time_of_services_formatted']);
...
function bkx_validate_form(source_val,destination_val)
{
	...
	if(source_val==3 && destination_val == 4)
	{
		...
		{
			var temp_err = bkx_ValidateEmail($("#id_email").val());
			if(temp_err != true)
			{
