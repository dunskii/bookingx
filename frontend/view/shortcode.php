...
{ 	
    ...
    if ( bkx_is_session_started() === FALSE ) session_start();

	$bkx_form_flag = "display:block;";
	...
	//session_start();
	require_once(BKX_PLUGIN_DIR_PATH.'inc/booking_general_functions.php'); //when needed
    ...
    $type_post_id = isset($_POST['id']) && $_POST['id']!= "" ? $_POST['id'] : '';
    $type_id= isset($type_id) && $type_id != 1 ? $type_id : sanitize_text_field ($type_post_id);
    $type_value = isset($_POST['type']) && $_POST['type']!= "" ? $_POST['type'] : '';
     
    if (isset($type) && $type!='' && ($type == 'seat' || sanitize_text_field ($type_value) =='bkx_seat')){ $selected_seat = $type_id;}
    else { $selected_seat ='zero';}

    if(isset($type) && $type!='' && $type == 'base' || sanitize_text_field ($type_value) =='bkx_base'){ $by_base = $type_id; }

	$option_for_link = bkx_crud_option_multisite('bkx_plugin_page_id');
	...
	$arr_seat = bkx_get_seat_list($by_base);        
	$arr_base = bkx_get_base_list();
    ...
    $dup_order_prevent = isset($_SESSION['dup_order_prevent']) && $_SESSION['dup_order_prevent']!= "" ? 1 : "";

	if (isset($_POST) && isset($_POST['is_submit_4']) && sanitize_text_field ($_POST['is_submit_4']) == 1 && sizeof($_POST) > 0 && $dup_order_prevent == '') {
	...
	if(isset($_POST['booking_multi_days']) && !empty($_POST['booking_multi_days']))
	{
 	...
 	$bookingObj->generate_order($arrData);
			...
			if($order_id){
				$bookingObj->add_order_note( sprintf( __( 'Booking has been successfully updated..', 'bookingx' ), ''), 0, null, $order_id );
			}else
			{
				$bookingObj->add_order_note( sprintf( __( 'Booking has been successfully created..', 'bookingx' ), ''), 0, null, $order_id );
			}
			...
			if(isset($order_id) && $order_id!='') {
				...
				if ($PayPalApiUsername != "" && $PayPalApiPassword != "" && $PayPalApiSignature != "" && $order_id != '') //if paypal credentials are set in the meta value options
				{
					...
					if (!$_REQUEST['flag_stop_redirect']) {
						bkx_do_paypal_api_call($base_name, $tot_prc, $order_id, $quantity, $arrPaypal);
						...
						if (isset($_POST['input_base'])) {
							...
							$base_price = ($BkxBaseObj->get_price() != '') ? bkx_get_current_currency() . $BkxBaseObj->get_price() : '';
				...
				} else //else direct user to success and send email for seat confirmation
				{
		...
		if (isset($_GET["token"]) && isset($_GET["PayerID"])) {
			...
			$httpParsedResponseAr = $paypal->bkx_PPHttpPost('DoExpressCheckoutPayment', 
			...
			if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
                ...
                */
				...
				$paypal = new BkxPayPalGateway();
				$httpParsedResponseAr = $paypal->bkx_PPHttpPost('GetTransactionDetails', $nvpStr, $PayPalApiUsername, $PayPalApiPassword, $PayPalApiSignature, $PayPalMode);
				...
				if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
					...
					//send email that booking confirmed
					//$res = bkx_do_send_mail($order_id);				
					$bkx_order_base_id = $BkxBookingObj['base_id'];
					if (isset($bkx_order_base_id)) {
						$base_id = $bkx_order_base_id;
	...
	$bkx_label_of_step1 =  (!empty(bkx_crud_option_multisite("bkx_label_of_step1"))) ? bkx_crud_option_multisite("bkx_label_of_step1") : 'Please select what you would like to book';
	$bkx_label_of_step1 = sprintf(esc_html__('%1$s','bookingx'), $bkx_label_of_step1 );
	$terms_page = "";
...
<form method="post" enctype="multipart/form-data" id="bkx_4" action="">
 <!-- Updated By :Divyang Parekh
        ...
        <div id="bkx_progressbar_wrapper_4" class="bkx_progressbar_wrapper customer_booking_form_wrapper">
                        ...
                        <div class="bkx_body"><div id="bkx_page_4_1" class="bkx_page">
                          <div class="bkx_page_fields">
                            <ul id="bkx_fields_4" class="bkx_fields top_label description_below"><li id="field_4_6" class="gfield  gsection"><h4 class="gsection_title">'.$bkx_label_of_step1.'</h4></li><li id="field_4_1" class="gfield"><label class="gfield_label" for="input_4_1">
		...
		<input type="button" id="bkx_customer_update"  class="bkx_customer_update" value="'.sprintf(__('Update Booking ','bookingx'), '').'">                   
                    ...
                    <div class="bkx_page_footer" id="bkx_page_footer_details" '.$want_to_disable.'>
                        <input type="button" id="bkx_previous_button_4_19" class="button bkx_previous_button" value="'.sprintf(__('Previous ','bookingx'), '').'" tabindex="24" onclick="jQuery(\'#bkx_target_page_number_4\').val(\'2\');bkx_validate_form(3,2);"> <input type="button" id="bkx_next_button_4_19" class="button bkx_next_button" value="'.sprintf(__('Next ','bookingx'), '').'" tabindex="23" onclick="jQuery(\'#bkx_target_page_number_4\').val(\'4\');bkx_validate_form(3,4);">
                    </div>
                </div>
		...
		<input type="button" id="bkx_previous_button_4" class="button bkx_previous_button" value="'.sprintf(__('Previous','bookingx'), '').'" tabindex="25" onclick="bkx_validate_form(4,3);">';
 ...
 $container_start = ' <div class="bookingx_form_container" style="'.$bkx_form_flag.'"> ';
 $container_end = ' </div> ';
	// $booking_data_live = $GLOBALS['bkx_booking_data'];
		//$order_id = $booking_data_live['meta_data']['order_id'];
 ...
 *@return String $temp
 */
function bkx_successform_shortcode_function()
{
	$temp = "Success";
	$r = new PayPal();
...
add_shortcode('successform', 'bkx_successform_shortcode_function');

add_shortcode('bookingx', 'bkx_process_bookingx');
/**
 ...
 * @param type $attr
 */
function bkx_process_bookingx($attr)
{ 
	 if ( is_admin() ) 
	 	return;
    ...
    if(!empty($attr))
    {
        foreach ($attr as $type=>$value)
        {
            ...
            switch ($type) {
                case 'seat-id':
                        bkx_generate_seat_data($attr);
                    break;
                 case 'base-id':
                        bkx_generate_base_data($attr);
                    break;
                 case 'extra-id':
                        bkx_generate_addition_data($attr);
                    break;
...
function bkx_generate_seat_data($data){
    $in_array = "";
    $post_type ='bkx_seat';
    $url_arg_name = 'seat';
    ...
    require_once BKX_PLUGIN_DIR_PATH.'frontend/templates/archive.php';
}

function bkx_generate_base_data($data){
    $in_array = "";
    $post_type ='bkx_base';
    $url_arg_name = 'base';
...
function bkx_generate_addition_data($data){
    $in_array = "";
...
function bkx_get_custom_post_type_template($single_template) {
     global $post;
    
     if ($post->post_type == 'bkx_seat' || $post->post_type =='bkx_base' || $post->post_type =='bkx_addition') {
          $single_template = BKX_PLUGIN_DIR_PATH . 'frontend/templates/single-'.$post->post_type.'.php';
     }
     return $single_template;
}
add_filter( 'single_template', 'bkx_get_custom_post_type_template' );